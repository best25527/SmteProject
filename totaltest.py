...
def search_api_station(keyword: str):
    ...
    try:
        ...
        # 🟢 เรียกใช้ normalize_thai_place ตัวเดียวกับระบบแรกตรงนี้!
        clean_kw = normalize_thai_place(keyword).lower().strip()
        ...
        for item in data:
            ...
            # 🟢 ตัดคำนำหน้าออกจากข้อมูล API ด้วย normalize_thai_place ก่อนเอามาเทียบ
            full_text = f"{st_id} {st_name} {province} {amphoe} {tambon}".lower()
            norm_full_text = normalize_thai_place(full_text)

            if clean_kw in norm_full_text:
                loc_details = []
                if amphoe: loc_details.append(f"อ./เขต {amphoe}")
    ...
    for event in events:
        if event.get("type") == "message" and event.get("message", {}).get("type") == "text":
            ...
            if raw_text == "เปิดระบบค้นหาสถานีน้ำ":
                ...
                reply_text(reply_token, "🔍 เปิดระบบค้นหา!\nกรุณาพิมพ์ชื่อสถานีน้ำที่คุณต้องการตรวจสอบ (ค้นหาได้ 1 ครั้งต่อการเรียกใช้)")
                continue
            ...
            if raw_text == "เปิดระบบพยากรณ์" or raw_text == "พยากรณ์":
                ...
                reply_text(reply_token, "🔮 เปิดระบบพยากรณ์ระดับน้ำ \nกรุณาพิมพ์ชื่อสถานี อำเภอ จังหวัด หรือชื่อคลองที่คุณต้องการดูผลพยากรณ์ 7 วันล่วงหน้า (พยากรณ์ได้ 1 ครั้งต่อการเรียกใช้)")
                continue
