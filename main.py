...
async def webhook(request: Request):
    ...
    for event in events:
        if event.get("type") == "message" and event.get("message", {}).get("type") == "text":
            ...
            if not station_id:
                reply_text(reply_token, f"❌ ไม่พบสถานีวัดระดับน้ำในพื้นที่ '{search_key}'\nกรุณาลองพิมพ์ระบุชื่อคลอง อำเภอ หรือจังหวัดใหม่อีกครั้งครับ")
                continue

            # 2. รัน AI พยากรณ์ (ใช้ Smart Hybrid System)
            try:
